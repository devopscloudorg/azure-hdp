...
if [[ -z $result ]]; then
	(azure storage account create --affinity-group "$affinityGroupName" --disable-geoReplication $storageAccount) || { echo "Failed to create storage account $storageAccount"; exit 1; }
else
